...
class DlibLitModule(LightningModule):
    ...
    def training_step(self, batch: Any, batch_idx: int):
        ...
        self.log("train/mae", self.train_mae, on_step=False, on_epoch=True, prog_bar=True) # change train/acc -> train mae
    ...
    def validation_step(self, batch: Any, batch_idx: int):
        loss, preds, targets = self.model_step(batch)
        # if batch_idx = 0 -> draw -> save pred, batch, ...
        # log out -> wandb
        x, y = batch
        img = TransformDataset.annotate_batch(x, preds)
        wandb_img = wandb.Image(img)
        # update and log metrics
        self.logger.experiment.log({"val/img": wandb_img})
        self.val_loss(loss)
